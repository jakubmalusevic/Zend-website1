 ...
 */
Ext.define('Glamme.GroupTabPanel', {
...
/*            plugins: [{
            ...
            columns: [{
                ...
                renderer: function (value, cell, node, idx1, idx2, store, tree) {
                    ...
                    if (node.parentNode.data.id != 'root') {
                        cls += ' ' + node.parentNode.data.id + ' ' + node.data.id;
                        if (window.aclStarted) {
                            if (window.disabledTabs[node.parentNode.data.id] && window.disabledTabs[node.data.id]) {
                                cls+=' hide';
                            }
                        } else {
                            window.disabledTabs[node.parentNode.data.id]=1;
                            window.disabledTabs[node.data.id]=1;
                            cls+=' hide';
                        }
                    } else {
                        cls += ' ' + node.data.id + ' ' + node.data.id + '-main';
                        if (window.aclStarted) {
                            if((window.disabledTabs[node.data.id]==1)&&(window.disabledTabs[node.data.id+'-main']==1)) {
                                cls+=' hide';
                            }
                        } else {
                            window.disabledTabs[node.data.id]=1;
                            window.disabledTabs[node.data.id+'-main']=1;
                            cls+=' hide';
                        }
                    }
    ...
    setActiveGroupAndTab: function(cmp, nodeId) {
        this.down('treepanel').getSelectionModel().select(this.store.getRootNode().findChild("id", nodeId, true));
        return true;
    },
    /**
     * @private
